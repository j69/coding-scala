...
import udemy.advanced1.module2.support.KoanSuite

class Module02My extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  test("Can create an empty Car of a certain type with Car.empty[TYPE]") {
    ...
    object Car {
      def empty[T <: TransportableItem](): Unit = {}
  ...
  test("Empty Car should store items of the right type and access them by index") {
    // Put your implementation here inside the test to prevent conflicting with other tests.
    class Car[T](items: Vector[T]) {
      def add(item: T) = new Car[T](this.items :+ item)

      def item(i: Int) = items.lift(i)

      def numberOfItems(): Int = items.size
    }

    object Car {
      def empty[T <: TransportableItem]() = new Car(Vector[T]())
  ...
  test("Car should be covariant in its type parameter") {
    // Put your implementation here inside the test to prevent conflicting with other tests.
    class Car[+T](items: Vector[T]) {
      def add[P >: T <: TransportableItem](item: P) = new Car[P](this.items :+ item)

      def item(i: Int): Option[T] = items.lift(i)
    ...
    val carWithToothpick = emptyCar.add(Toothpick("toothpick-1"))
    carWithToothpick.toString should be("Toothpick(toothpick-1)")
    carWithToothpick.numberOfItems should be(1)
    carWithToothpick.item(0) should be(Some(Toothpick("toothpick-1")))
    carWithToothpick.item(1) should be(None)

    val carWithToothpickAndPiano = carWithToothpick.add(Piano("Yamaha"))
    carWithToothpickAndPiano.toString should be("Toothpick(toothpick-1), Piano(Yamaha)")
    carWithToothpickAndPiano.numberOfItems should be(2)
    carWithToothpickAndPiano.item(0) should be(Some(Toothpick("toothpick-1")))
    carWithToothpickAndPiano.item(1) should be(Some(Piano("Yamaha")))
    carWithToothpickAndPiano.item(2) should be(None)
  ...
  test("Car should work as expected with an abstract trait separated out for its API") {
    ...
    trait Car[+T <: TransportableItem] {
      def add[P >: T <: TransportableItem](item: P): Car[P]

      def item(i: Int): Option[T]
    ...
    object Car {
      // Put your implementation here inside the test to prevent conflicting with other tests.
      class CarImpl[+T <: TransportableItem](items: Vector[T]) extends Car[T] {
        def add[P >: T <: TransportableItem](item: P) = new CarImpl[P](this.items :+ item)
      ...
      def empty[T <: TransportableItem](): Car[T] = new CarImpl(Vector[T]())
    }
